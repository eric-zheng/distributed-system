...
public class ECSClient {
	                    ...
	                    if (event.getState() == Event.KeeperState.SyncConnected) {
	                    	try {
								...
								for(String prevact:activeList){
									...
									if(!found){			//in prev not in current, put in hibernation list, and remove all related nodes in the zookeeper
	...
	private boolean runServer(String name,int cSize, String policy){
		...
		try{
			...
			process=Runtime.getRuntime().exec("ssh -n "+host+" nohup java -jar "+workingDirectory+"/ms3-server.jar "+port+" "+cSize+" "+policy+" "+name+" "+ip+":2181 &");
		}
		catch(Exception e){
			e.printStackTrace();
		}
	...
	public boolean initService(int nodeNum,int cSize,String policy){
		...
		if(hibernationList.size()<nodeNum){
			System.out.println("Not enough avaliable machines. Current avaliable number of machines: "+hibernationList.size());
			return false;
		}
		...
		try{
			...
			while(numStarted<nodeNum){
				...
				if(runServer(name, cSize, policy)){		//successfully started the server
					...
					System.out.println("WARNING: Server "+name+" is not avaliable. Please check if the address:port is available. Address removed from the configuration list.");
				}
			}
		...
		initialized=true;
		System.out.println("Service initiated: nodeNum="+numStarted+", cacheSize="+cSize+", replacementPolicy="+policy);
		return true;
	}
	...
	public void moveData(String sender,String receiver, String startRange, String endRange) throws KeeperException, InterruptedException{
	...
	public void removeServerRange(String name,String startRange,String endRange) throws KeeperException, InterruptedException{
	...
	public void recovery(ArrayList<String> active) throws KeeperException, InterruptedException{			//for every current node,asks all the other nodes for the data in its replication range
