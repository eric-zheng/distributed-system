...
public class Table{
	...
	public boolean keepRow(int row, String whereStatement){		
		...
		for(int i=0;i<lexime.size();i++){
			if(lexime.get(i).type.equals("var")){
				...
				if(index==-1){
					...
					return true;		//true means we are keeping the row since the vars are invalid so we will do nothing
				}
		...
		//the final reduction makes sure that there is only one item left in the array(a simple boolean)
		if(reducer.size()!=1){
