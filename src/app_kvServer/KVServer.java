...
public class KVServer extends Thread implements Watcher {
		...
		if (serverSocket != null) {
			while (true) { // isRunning()
				try {
					...
					ClientConnection connection = new ClientConnection(client, s, isLocked, running, md, zname, zk);
	...
	private boolean initializeStorage() throws IOException {
		...
		logger.info("Set up storage with cache size: " + cacheSize + "; replacement strategy: " + strategy);
		return true;
	...
	/**
	 * Connect the KVServer to the zookeeper and create new node(s) on the znode
	 * tree The function also adds multiple watchers on proper znodes
	 * 
	 * @return true if success; false if fail
	 * @throws IOException
	...
	/**
	 * Set a watcher to response for remove data command from zookeeper Delete the
	 * data stored on disk within the range given via the content of rmdata node
	 */
		...
		b = zk.getData(path_rmdata, new Watcher() {
			public void process(WatchedEvent event) {
				if (event.getType() == Event.EventType.NodeDataChanged) {
					...
					System.out
							.println("Wrong! The data at rmdata node is not " + "changed but the watcher is invoked.");
				}
			}
		}, null);
	}
		...
		b = zk.getData(path_shutdown, new Watcher() {
			public void process(WatchedEvent event) {
				if (event.getType() == Event.EventType.NodeDataChanged) {
					...
					System.out.println(
							"Wrong! The data at shutdown node is not " + "changed but the watcher is invoked.");
				}
			}
		}, null);
	}
	...
	/**
	 * Used to set a watcher on the state znode of each KVServer, to support start
	 * and stop command
	 * 
	 * @throws IOException
	 * @throws InterruptedException
		...
		state = zk.getData(path_state, new Watcher() {
			public void process(WatchedEvent event) {
				if (event.getType() == Event.EventType.NodeDataChanged) {
					...
					System.out.println("Wrong! The data at state node is not " + "changed but the watcher is invoked.");
				}
			}
		}, null);
	}
	...
	void debug_put_data_on_znode() {
		...
		try {
			...
			zk.create(("/" + zname + "/debug"), null, Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
		...
		b = zk.getData(path_updatemd, new Watcher() {
			public void process(WatchedEvent event) {
				if (event.getType() == Event.EventType.NodeDataChanged) {
					...
					System.out.println(
							"Wrong! The data at updatemd node is not " + "changed but the watcher is invoked.");
				}
			}
		}, null);
	}
		...
		b = zk.getData(("/" + zname + "/lock"), new Watcher() {
			public void process(WatchedEvent event) {
				if (event.getType() == Event.EventType.NodeDataChanged) {
					...
					System.out.println("Wrong! The data at lock node is not " + "changed but the watcher is invoked.");
				}
			}
		}, null);
	}
		...
		b = zk.getData(("/" + zname + "/migrate"), new Watcher() {
			public void process(WatchedEvent event) {
				if (event.getType() == Event.EventType.NodeDataChanged) {
					...
					System.out
							.println("Wrong! The data at migrate node is not " + "changed but the watcher is invoked.");
				}
			}
		}, null);
	}
	...
	public void moveData(String start_range, String end_range, String receiver) {
		...
		for (DBResponse data : result) {
			...
			if (newMsg.getStatus().equals(StatusType.PUT_SUCCESS) || newMsg.getStatus().equals(StatusType.PUT_UPDATE)) {
	 ...
	 * Main function
	 * 
	 * @param args contains the port number at args[0], cache size at args[1],
	 *             strategy at args[2], corresponding znode name of this KVServer at
	 *             args[3], zookeeper address and port number at args[4].
	 */
	public static void main(String[] args) {
		try {
			...
			if (args.length != 5) {
				...
				System.out.println("Usage: Server <port> <cache size> " + "<cache strategy: FIFO, LRU, LFU> "
