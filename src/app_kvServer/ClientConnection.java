...
/**
 * Connection end point for each client connected to server.
 * Used to receive, process and send data with client.
 */
public class ClientConnection implements Runnable {
	 ...
	 */
	public ClientConnection(Socket clientSocket, Storage s, MutableBoolean isLocked, 
			MutableBoolean isRunning, Metadata md, String zname, ZooKeeper zk) {
	...
	void debug_put_data_on_znode() {
		...
		try {
			...
			zk.create(("/" + serverName + "/debug"), null, Ids.OPEN_ACL_UNSAFE,
...
/*// Add server log info
if (rtnmsg.getStatus().equals(StatusType.DELETE_ERROR)) {
	logger.info("SEND \t<" 
			+ clientSocket.getInetAddress().getHostAddress() + ":" 
			+ clientSocket.getPort() + ">: '" 
			+ "DELETE_ERROR" + " "
