...
public class Storage {
	 ...
	 *
	 * 
	 * !!!Important assumption!!!: we are assuming that a string should not contain
	 * a null character ever! which makes sense. Also we are assuming that the
	 * string is within correct range of size
	 */
	 ...
	 * @throws IOException
	 */
	private DBResponse getTuple(String source) throws IOException { // get a tuple from the disk. If used consecutively
																	// it will continue to get the next tuple
		File tempFile = new File(source);
		if (!(tempFile.exists() && tempFile.isFile())) { // if the file does not exist then create one!
			tempFile.createNewFile();
		}
	...
	private boolean removeTuple(String key) throws IOException { // very high cost! We definitely need better design at
																	// the persistent file system but not now!
	 ...
	 * @return a DBResponse object contains the key-value pair. Also indicating if
	...
	/*
	 * TESTING SECTION: ONLY USED FOR DEBUG PURPOSE. THE USER DO NOT NEED TO USE
	 * THESE FUNCTIONS
	 * 
	 **/
	 ...
	 * Instructions:
	 * 
	 * g ky: try to get value with key=ky p ky value: store the key value pair in
	 * the DB d ky: delete all existing records with key=ky q: exit the shell
	 */

	/*
	 * public static void main(String []args)throws IOException{ Storage s=new
	 * Storage(10, CacheType.FIFO);
	 * 
	 * s.put("123","123"); s.put("456","456");
	 * 
	 * System.out.println(s.getMD5String("123"));
	 * System.out.println(s.getMD5String("456"));
	 * 
	 * ArrayList<DBResponse> res=s.getInRange("250CF8B51C773F3F8DC8B4BE867A9A02",
	 * "250CF8B51C773F3F8DC8B4BE867A9A02");
	 * 
	 * for(DBResponse d:res){ System.out.println("key: "+d.key+", value: "+d.value);
	 * }
	 * 
	 * s.removeNotInRange("250CF8B51C773F3F8DC8B4BE867A9A01",
